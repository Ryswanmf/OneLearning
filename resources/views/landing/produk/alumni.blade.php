...
@section('title', 'Tryout Alumni - Persiapan UTBK & Mandiri | OneLearning')
    ...
    <section id="daftar-tryout" class="py-24 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-3xl md:text-5xl font-black text-secondary tracking-tight">Paket <span class="text-primary italic">Alumni Premium</span></h2>
                <p class="text-secondary/50 font-medium mt-4">Analisis mendalam untuk memastikan kesiapanmu di ujian berikutnya.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500 group relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                        <span class="inline-block px-3 py-1 bg-secondary/10 text-secondary font-black text-[10px] uppercase tracking-widest rounded-lg mb-4">{{ $tryout->subject ?? 'Mata Pelajaran' }}</span>
                        ...
                        <div class="flex items-center justify-between mt-8">
                            ...
                            @auth
                                ...
                                @endphp
                                @if($hasAccess)
                                    <a href="{{ route('tryout.instructions', ['type' => 'alumni', 'id' => $tryout->slug]) }}" class="px-6 py-3 bg-secondary text-white font-black text-xs uppercase tracking-widest rounded-xl hover:bg-primary transition-all">Kerjakan</a>
                                @else
                                    <a href="{{ route('order.checkout', ['type' => 'alumni', 'id' => $tryout->slug]) }}" class="px-6 py-3 bg-primary text-white font-black text-xs uppercase tracking-widest rounded-xl hover:bg-secondary transition-all">Beli Paket</a>
                                @endif
                            @else
