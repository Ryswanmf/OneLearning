...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-xs font-medium text-secondary/40 mt-1">Buat program kemitraan atau layanan B2B baru.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.bisnis.store') }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    <x-input-label for="title" value="Nama Layanan / Program" />
            ...
            <div class="space-y-2">
                ...
                <textarea id="description" name="description" rows="5" class="block w-full bg-gray-50/50 border-gray-100 focus:border-primary focus:ring-primary/10 rounded-2xl p-4 font-bold text-xs transition-all" placeholder="Jelaskan detail layanan atau program ini...">{{ old('description') }}</textarea>
            </div>
        </div>
