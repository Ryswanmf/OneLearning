...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-black text-secondary tracking-tight">Ubah <span class="text-primary italic">Layanan Bisnis</span>.</h2>
            <p class="text-xs font-medium text-secondary/40 mt-1">Perbarui informasi layanan <strong>{{ $service->title }}</strong>.</p>
        </div>
    </div>

    <form action="{{ route('admin.layanan-bisnis.update', $service->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf @method('PUT')
        <div class="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-4">
                    ...
                    <div class="relative w-24 h-24 rounded-2xl overflow-hidden border border-gray-100 mb-2 group">
                        <img src="{{ filter_var($service->image, FILTER_VALIDATE_URL) ? $service->image : asset('storage/' . $service->image) }}" class="w-full h-full object-cover">
                        <div class="absolute inset-0 bg-secondary/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <span class="text-[8px] font-black text-white uppercase tracking-widest text-center px-1">Gambar Saat Ini</span>
                        </div>
                    </div>
                    @endif
            ...
            <div class="space-y-2">
                ...
                <textarea id="description" name="description" rows="5" class="block w-full bg-gray-50/50 border-gray-100 focus:border-primary focus:ring-primary/10 rounded-2xl p-4 font-bold text-xs transition-all">{{ old('description', $service->description) }}</textarea>
            </div>
        </div>
