...
@section('title', 'Tambah Future Educators - OneLearning')
...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-black text-secondary tracking-tight">Tambah <span class="text-primary italic">Program Pendidik</span>.</h2>
            <p class="text-xs font-medium text-secondary/40 mt-1">Buat inisiatif baru untuk komunitas guru dan pengajar.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.future-educators.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8">
            <div class="space-y-2">
                ...
                <x-text-input id="title" name="title" type="text" :value="old('title')" required placeholder="Contoh: Webinar Kurikulum Merdeka" />
            </div>
            ...
            <div class="space-y-2">
                ...
                <textarea id="description" name="description" rows="5" class="block w-full bg-gray-50/50 border-gray-100 focus:border-primary focus:ring-primary/10 rounded-2xl p-4 font-bold text-xs transition-all" placeholder="Jelaskan manfaat dan tujuan program ini bagi para pendidik...">{{ old('description') }}</textarea>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-4">
            ...
            <button type="submit" class="px-10 py-4 bg-primary text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-primary/20 hover:bg-secondary transition-all"> Simpan Program </button>
        </div>
    </form>
</div>
@endsection
