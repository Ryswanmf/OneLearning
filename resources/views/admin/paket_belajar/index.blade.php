...
@section('title', 'Kelola Paket Belajar - OneLearning')

@section('content')
<div class="space-y-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="text-3xl font-black text-secondary tracking-tight">Daftar <span class="text-primary italic">Paket Belajar</span>.</h2>
            <p class="text-sm font-medium text-secondary/40 mt-1">Kelola harga dan fitur paket berlangganan untuk siswa.</p>
        </div>
        <a href="{{ route('admin.paket-belajar.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-primary text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-secondary shadow-xl shadow-primary/20 transition-all active:scale-95">
                    ...
                    <tr class="group hover:bg-gray-50/30 transition-all">
                        ...
                        <td class="px-8 py-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.questions.index', ['type' => 'paket', 'id' => $package->slug]) }}" class="p-2 text-secondary/20 hover:text-accent transition-colors" title="Kelola Bank Soal">
                                ...
                                <form action="{{ route('admin.paket-belajar.destroy', $package->slug) }}" method="POST" onsubmit="return confirm('Hapus paket ini?')">
                                    @csrf @method('DELETE')
