...
@section('title', 'Tambah Tryout SMP - OneLearning')
...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-black text-secondary tracking-tight">Tambah <span class="text-primary italic">Tryout SMP</span>.</h2>
            <p class="text-xs font-medium text-secondary/40 mt-1">Buat simulasi ujian baru untuk jenjang Sekolah Menengah Pertama.</p>
        </div>
    </div>

    <form action="{{ route('admin.smp.store') }}" method="POST" class="space-y-6">
        @csrf
        <div class="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <x-text-input id="name" name="name" type="text" :value="old('name')" required placeholder="Contoh: Persiapan Ujian Tengah Semester #1" />
                </div>
                <div class="space-y-2">
                    ...
                    <select id="subject" name="subject" class="block w-full bg-gray-50/50 border-gray-100 focus:border-primary focus:ring-primary/10 rounded-xl h-12 px-4 font-bold text-xs transition-all">
                        ...
                        <option value="Campuran">Campuran / Asesmen Nasional</option>
                    </select>
                </div>
            </div>
