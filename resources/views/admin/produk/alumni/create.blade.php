...
@section('title', 'Tambah Tryout Alumni - OneLearning')

@section('content')
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-center gap-4">
        <a href="{{ route('admin.alumni.index') }}" class="p-2 bg-white border border-gray-100 rounded-xl text-secondary hover:text-primary transition-all shadow-sm">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
        </a>
        <div>
            <h2 class="text-2xl font-black text-secondary tracking-tight">Tambah <span class="text-primary italic">Tryout Alumni</span>.</h2>
            <p class="text-xs font-medium text-secondary/40 mt-1">Buat simulasi ujian baru khusus untuk kategori alumni (Gapyear/Linjur).</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.alumni.store') }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <x-text-input id="name" name="name" type="text" :value="old('name')" required placeholder="Contoh: Tryout Spesial Alumni #1" />
                </div>
                <div class="space-y-2">
                    <x-input-label for="subject" value="Mata Pelajaran / Program" />
                    <select id="subject" name="subject" class="block w-full bg-gray-50/50 border-gray-100 focus:border-primary focus:ring-primary/10 rounded-xl h-12 px-4 font-bold text-xs transition-all">
                        ...
                        <option value="CPNS">CPNS (SKD/SKB)</option>
        ...
        <div class="flex justify-end gap-4">
            <a href="{{ route('admin.alumni.index') }}" class="px-8 py-4 text-xs font-black text-secondary/40 uppercase tracking-widest hover:text-secondary transition-all">Batal</a>
