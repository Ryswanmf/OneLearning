...
@section('title', 'Kelola Tryout SMA 12 & UTBK - OneLearning')

@section('content')
<div class="space-y-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h2 class="text-3xl font-black text-secondary tracking-tight">Tryout <span class="text-primary italic">SMA 12 & UTBK</span>.</h2>
            <p class="text-sm font-medium text-secondary/40 mt-1">Kelola bank soal dan simulasi ujian untuk persiapan masuk perguruan tinggi.</p>
        </div>
        <a href="{{ route('admin.sma-utbk.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-primary text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-secondary shadow-xl shadow-primary/20 transition-all active:scale-95">
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="group hover:bg-gray-50/30 transition-all">
                        ...
                        <td class="px-8 py-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.questions.index', ['type' => 'sma-utbk', 'id' => $tryout->id]) }}" class="p-2 text-secondary/20 hover:text-accent transition-colors" title="Kelola Bank Soal">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
                                </a>
                                <a href="{{ route('admin.sma-utbk.edit', $tryout->id) }}" class="p-2 text-secondary/20 hover:text-primary transition-colors">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                </a>
                                <form action="{{ route('admin.sma-utbk.destroy', $tryout->id) }}" method="POST" onsubmit="return confirm('Hapus paket ini?')">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-8 py-20 text-center">
                            <p class="text-sm font-bold text-secondary/30">Belum ada paket tryout SMA 12 & UTBK.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
