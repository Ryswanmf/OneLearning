...
@section('title', 'Ubah Tryout SD - OneLearning')
...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-black text-secondary tracking-tight">Ubah <span class="text-primary italic">Tryout SD</span>.</h2>
            <p class="text-xs font-medium text-secondary/40 mt-1">Perbarui parameter simulasi ujian <strong>{{ $tryout->name }}</strong>.</p>
        </div>
    </div>

    <form action="{{ route('admin.sd.update', $tryout->id) }}" method="POST" class="space-y-6">
        @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <select id="subject" name="subject" class="block w-full bg-gray-50/50 border-gray-100 focus:border-primary focus:ring-primary/10 rounded-xl h-12 px-4 font-bold text-xs transition-all">
                        ...
                        <option value="IPAS" {{ $tryout->subject == 'IPAS' ? 'selected' : '' }}>IPAS</option>
                        <option value="Campuran" {{ $tryout->subject == 'Campuran' ? 'selected' : '' }}>Campuran / Ujian Sekolah</option>
                    </select>
                </div>
            </div>
