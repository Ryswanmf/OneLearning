...
@section('title', 'Ubah Tryout UTBK - OneLearning')
...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-black text-secondary tracking-tight">Ubah <span class="text-primary italic">Tryout UTBK</span>.</h2>
            <p class="text-xs font-medium text-secondary/40 mt-1">Perbarui parameter simulasi ujian <strong>{{ $tryout->name }}</strong>.</p>
        </div>
    </div>

    <form action="{{ route('admin.utbk.update', $tryout->id) }}" method="POST" class="space-y-6">
        @csrf @method('PUT')
        <div class="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <select id="category" name="category" class="block w-full bg-gray-50/50 border-gray-100 focus:border-primary focus:ring-primary/10 rounded-xl h-12 px-4 font-bold text-xs transition-all">
                        <option value="TPS" {{ $tryout->category == 'TPS' ? 'selected' : '' }}>TPS (Tes Potensi Skolastik)</option>
                        <option value="Literasi" {{ $tryout->category == 'Literasi' ? 'selected' : '' }}>Literasi (B.Indo & B.Inggris)</option>
                        <option value="Penalaran" {{ $tryout->category == 'Penalaran' ? 'selected' : '' }}>Penalaran Matematika</option>
                        <option value="Campuran" {{ $tryout->category == 'Campuran' ? 'selected' : '' }}>Full Simulasi (Campuran)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <x-text-input id="price" name="price" type="number" :value="old('price', $tryout->price)" required />
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <select id="status" name="status" class="block w-full bg-gray-50/50 border-gray-100 focus:border-primary focus:ring-primary/10 rounded-xl h-12 px-4 font-bold text-xs transition-all">
                    <option value="draft" {{ $tryout->status == 'draft' ? 'selected' : '' }}>Draft (Belum Rilis)</option>
                    <option value="published" {{ $tryout->status == 'published' ? 'selected' : '' }}>Published (Siap Dikerjakan)</option>
                </select>
            </div>
        </div>
