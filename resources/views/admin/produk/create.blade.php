...
@section('title', 'Tambah Produk - OneLearning')
...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-black text-secondary tracking-tight">Tambah <span class="text-primary italic">Produk Baru</span>.</h2>
            <p class="text-xs font-medium text-secondary/40 mt-1">Lengkapi data di bawah ini untuk menambahkan program belajar baru.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.produk.store') }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <x-text-input id="title" name="title" type="text" :value="old('title')" required placeholder="Contoh: UTBK-SNBT 2024" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <x-text-input id="package_count" name="package_count" type="number" :value="old('package_count', 0)" required />
                    <x-input-error :messages="$errors->get('package_count')" class="mt-1" />
                </div>
                <div class="space-y-2">
                    <x-input-label for="duration" value="Masa Aktif" />
                    <x-text-input id="duration" name="duration" type="text" :value="old('duration', '1 Tahun')" required />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <x-text-input id="image" name="image" type="text" :value="old('image')" placeholder="https://images.unsplash.com/..." />
                    <p class="text-[10px] text-secondary/30 font-medium italic">Masukkan URL gambar (Unsplash/Imgur) untuk sementara.</p>
                </div>
            ...
            <div class="space-y-2">
                ...
                <textarea id="description" name="description" rows="4" class="block w-full bg-gray-50/50 border-gray-100 focus:border-primary focus:ring-primary/10 rounded-2xl p-4 font-bold text-xs transition-all placeholder:text-secondary/20">{{ old('description') }}</textarea>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-4">
            ...
            <button type="submit" class="px-10 py-4 bg-primary text-white font-black text-xs uppercase tracking-widest rounded-2xl shadow-xl shadow-primary/20 hover:bg-secondary transition-all active:scale-95"> Simpan Produk </button>
        </div>
    </form>
</div>
@endsection
